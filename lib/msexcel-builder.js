...
SheetRelation = (function() {

	function SheetRelation(book) {
		this.book = book;
	}

	SheetRelation.prototype.toxml = function(index) {
		var i, rs, _ref;
		rs = xml.create('Relationships', {
			version: '1.0',
			encoding: 'UTF-8',
		...
		rs.att('xmlns', 'http://schemas.openxmlformats.org/package/2006/relationships');
		for (i = 0, _ref = this.book.sheets[index].hyperlinks.length; i < _ref ; i++) {
			rs.ele('Relationship', {
				Id: 'rId' + (i + 1),
				Type: 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink',
				Target: this.book.sheets[index].hyperlinks[i].url || 'http://upsa.epam.com/workload/employeeView.do?employeeId=4060741400006008633',
				TargetMode: 'External'
			});
		}
		return rs.end();
...
Sheet = (function() {
	...
	Sheet.prototype.set = function(col, row, str, type) {
		if ((str != null) && str !== '') {
			this.data[row][col].type = (type && "n|s".indexOf(type) != -1 ? type : "s");
			return this.data[row][col].v = (this.data[row][col].type === "s" ? this.book.ss.str2id('' + str) : str);
		}
	};

	Sheet.prototype.link = function(col, row, linkObj) {
		return this.hyperlinks.push({
			col: col,
			row: row,
			text: this.set(col, row, linkObj.text, linkObj.textType),
			url: linkObj.url
		});
	};
	...
	Sheet.prototype.toxml = function() {
		var c, cols, cw, ht, i, ix, j, m, mc, h, hl, r, sd, sid, ws, _i, _j, _len, _len2, _ref, _ref2, _ref3, _ref4, _ref5;
		...
		for (i = 1, _ref2 = this.rows; 1 <= _ref2 ? i <= _ref2 : i >= _ref2; 1 <= _ref2 ? i++ : i--) {
			...
			for (j = 1, _ref3 = this.cols; 1 <= _ref3 ? j <= _ref3 : j >= _ref3; 1 <= _ref3 ? j++ : j--) {
				...
				if (ix.type) {
					...
					if (ix.type) {
						c.att('t', ix.type);
						(ix.type === 's' ? c.ele('v', '' + (ix.v - 1)) : c.ele('v', '' + ix.v ));
					}
				}
			}
		}
		...
		if (this.hyperlinks.length > 0) {
			hl = ws.ele('hyperlinks');
			_ref5 = this.hyperlinks;
			for (_j = 0, _len2 = _ref5.length; _j < _len2; _j++) {
				h = _ref5[_j];
				hl.ele('hiperlink', {
					ref: '' + tool.i2a(h.col) + h.row,
					'r:id': 'rId'+(_j + 1)
				});
			}
		}
...
Style = (function() {
	...
	Style.prototype.font2id = function(font) {
		...
		font.name || (font.name = 'Times New Roman');
	...
	Style.prototype.toxml = function() {
		...
		var cellStylesTag = ss.ele('cellStyles', {
			count: '1'
		});
		cellStylesTag.ele('cellStyle', {
			// name: 'Normal',
			xfId: '0'//,
			// builtinId: '0'
		});
...
Workbook = (function() {
	...
	Workbook.prototype.save = function(cb) {
		var i, target, _ref, sr, sheetList = [], sheetrelsList = [];
		var self = this;
		target = path.join(this.fpath, this.id);
		...
		for (i = 0, _ref = this.sheets.length; 0 <= _ref ? i < _ref : i > _ref; 0 <= _ref ? i++ : i--) {
			...
			if(this.sheets[i].hyperlinks.length){
				sr = new SheetRelation(this);
				sheetrelsList.push({name: target + '/xl/worksheets/_rels/sheet' + (i + 1) + '.xml.rels', data: sr.toxml(i)});
			}
		}
		...
		async.series([
			...
			async.apply(fs.mkdir, target + '/xl/worksheets/_rels'),
			...
			function(cb){
				async.forEach(sheetrelsList, function(sheetrel, sheetDone) {
					fs.writeFile(sheetrel.name, sheetrel.data, sheetDone);
				}, function(err) {
					cb(err);
				});
			}
